...
mod tests {
    ...
    fn numbers() {
        ...
        assert_eq!(eval(&one, Verbosity::Normal).unwrap(), Expr::Num(1));
        assert_eq!(type_check(&one).unwrap(), Type::Num);
        let num = parse("1234567").unwrap();
        assert_eq!(eval(&num, Verbosity::Normal).unwrap(), Expr::Num(1234567));
        assert_eq!(type_check(&num).unwrap(), Type::Num);
        let zero = parse("0").unwrap();
        assert_eq!(eval(&zero, Verbosity::Normal).unwrap(), Expr::Num(0));
    ...
    fn simple_arithmetic() {
        ...
        assert_eq!(eval(&add, Verbosity::Normal).unwrap(), Expr::Num(3));
        assert_eq!(type_check(&add).unwrap(), Type::Num);
        let sub = parse("1-2").unwrap();
        assert_eq!(eval(&sub, Verbosity::Normal).unwrap(), Expr::Num(-1));
        assert_eq!(type_check(&sub).unwrap(), Type::Num);
        let mul = parse("1*2").unwrap();
        assert_eq!(eval(&mul, Verbosity::Normal).unwrap(), Expr::Num(2));
        assert_eq!(type_check(&mul).unwrap(), Type::Num);
        let div = parse("1/2").unwrap();
        assert_eq!(eval(&div, Verbosity::Normal).unwrap(), Expr::Num(0));
    ...
    fn complex_arithmetic() {
        ...
        assert_eq!(eval(&expr, Verbosity::Normal).unwrap(), Expr::Num(1));
