...
mod tests {
    ...
    fn aequiv() {
        ...
        assert!(Expr::alpha_equiv(
            ...
            *parse("case (inj 1=L) {L(n)->n+1|R(r_)->3*r_}").unwrap()
        ));
        assert!(Expr::alpha_equiv(
            expr1.clone().substitute(Variable::from("r"), Expr::Var("n".into())),
            *parse("case (inj 1=L) {L(l_)->n+1|R(n)->3*n}").unwrap()
        ));
