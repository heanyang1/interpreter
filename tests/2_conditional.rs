...
mod tests {
    ...
    fn ifelse() {
        ...
        assert_eq!(eval(&expr1, Verbosity::Normal).unwrap(), Expr::Num(1));
        ...
        assert_eq!(eval(&expr3, Verbosity::Normal).unwrap(), Expr::Num(6));
    ...
    fn complex_relop() {
        ...
        assert_eq!(eval(&expr1, Verbosity::Normal).unwrap(), Expr::False);
        assert_eq!(type_check(&expr1).unwrap(), Type::Bool);
        let expr2 = parse("if 1<2*3 then 3==4+1 else 4>5").unwrap();
        assert_eq!(eval(&expr2, Verbosity::Normal).unwrap(), Expr::False);
