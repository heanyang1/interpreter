...
freshTypeVar = do
    n <- get
    modify (+1)
    return $ TVar (Variable ("type_" ++ show n))
...
getConstraints expr ctx = case expr of
    ...
    EIf cond then_ else_ -> do
        ...
        let constraints = cCond ++ cThen ++ cElse ++
    ...
    EOr left right -> do
        ...
        let constraints = cLeft ++ cRight ++
                [Constraint tauLeft TBool (show left) "Bool",
                 Constraint tauRight TBool (show right) "Bool"]
        return (TBool, constraints)
    EVar x -> lift $ Left $ "Free variable: " ++ show x
    EDeBruijn depth -> do
...
substituteMapVar mp t = case t of
    ...
    TForall a tau' -> if a `Map.member` mp
